...
class map_t:
    ...
    def grid_cell_from_xy(s, x, y):
        ...
        #### TODO: XXXXXXXXXXX
        #pdb.set_trace()
        x_ind = np.array((x-s.xmin)/s.resolution).astype(int)
        y_ind = np.array((y-s.ymin)/s.resolution).astype(int)
        valids = np.logical_and(np.logical_and(np.logical_and(x_ind>=0,x_ind<=s.szx),y_ind>=0),y_ind<=s.szy)
        x_ind = x_ind[valids]
        y_ind = y_ind[valids]
        x_ind = np.clip(x_ind,0,s.szx-1)
        y_ind = np.clip(y_ind,0,s.szy-1)
        return x_ind,y_ind
...
class slam_t:
    ...
    def stratified_resampling(p, w):
        ...
        #print("Resampling")
        for j in range(n):
            u = np.random.random(size=1)/n+j/n
            while(u>c):
                idx+=1
                c+=w[idx]
            p_strat[:,j] = p[:,idx]

        w_strat = np.ones((n,))/float(n)
        return p_strat,w_strat
    ...
    def rays2world(s, p, d, head_angle=0, neck_angle=0, angles=None):
        ...
        W_T_B = euler_to_se3(0,0,p[2],np.array([p[0],p[1],0]))
        lidar_points_body = d
        lidar_points_world = W_T_B@lidar_points_body

        return lidar_points_world[:2,:]

    def lidar2body(s,d,head_angle,neck_angle,angles = None):
        # 1. from lidar distances to points in the LiDAR frame
        # Converts lidar points to points in the body frame
        assert len(d)==len(s.lidar_angles)
        lidar_points = np.zeros((4,len(d)))
        lidar_points[0,:] = d*np.cos(angles)
        lidar_points[1,:] = d*np.sin(angles)
        lidar_points[2,:] = np.zeros(len(d))
        lidar_points[3,:] = np.ones(len(d))
        
        # 2. from LiDAR frame to the body frame
        lidar_points_body = np.zeros((4,len(d)))
  
        # joint_t = s.find_joint_t_idx_from_lidar(s.lidar[j]['t'])
        # pitch = s.joint['head_angles'][joint_name_to_index['Neck'], joint_t] 
        # yaw = s.joint['head_angles'][joint_name_to_index['Head'], joint_t]
        pitch = head_angle
        yaw = neck_angle
        roll = 0
        v = np.array([0, 0, s.lidar_height+s.head_height])
        B_T_lidar = euler_to_se3(roll,pitch,yaw,v)
        lidar_points_body = B_T_lidar@lidar_points  
        
        return lidar_points_body[:,lidar_points_body[2]>0]       
    ...
    def dynamics_step(s, t):
        ...
        noise = np.random.multivariate_normal(mean=[0,0,0], cov=s.Q, size=s.n)
        u = s.get_control(t)
        for j in range(s.p.shape[1]):
            s.p[:,j] = smart_plus_2d(s.p[:,j],u)
            s.p[:,j] = smart_plus_2d(s.p[:,j],noise[j,:])
            #s.n = s.n*
    ...
    def update_weights(w, obs_logp):
        ...
        w_ = np.log(w) + obs_logp
        w = np.exp(w_- slam_t.log_sum_exp(w_))

        assert np.abs((w.sum()-1))<=0.01
        return w
    ...
    def observation_step(s, t):
        ...
        d = s.lidar[t]['scan']
        log_ods = np.zeros((s.n,))
        d_body = s.lidar2body(d,head_angle=head_angle,neck_angle=neck_angle,angles=s.lidar_angles)
        #Projecting lidar scan and obtaining log prob of scan given particle
        for i in range(s.p.shape[1]):
            d_world = s.rays2world(s.p[:,i],d_body,head_angle=head_angle,neck_angle=neck_angle,angles=s.lidar_angles)
            cell_indx,cell_indy = s.map.grid_cell_from_xy(d_world[0],d_world[1])
            log_ods[i] = np.sum(s.map.cells[cell_indx,cell_indy])

        #Updating weights
        if s.p.shape[1]==1:
            pos_indx,pos_indy = s.map.grid_cell_from_xy(s.p[0],s.p[1])
            
            contours = np.vstack((cell_indy,cell_indx))
            contours = np.hstack((contours,np.array([pos_indy,pos_indx])))
            mask = np.zeros(s.map.log_odds.shape)

            cv.drawContours(image=mask,contours = [contours.T],contourIdx = -1,color = -s.map.log_odds_thresh,thickness = -1)
            s.map.log_odds = mask

            s.map.log_odds[cell_indx,cell_indy]+=2*s.map.log_odds_thresh

            s.map.cells[s.map.log_odds>=s.map.log_odds_thresh] = 1
            s.map.cells[s.map.log_odds<s.map.log_odds_thresh] = 0
        else:    
            s.w = slam_t.update_weights(s.w,log_ods)
            max_weight = s.w.argmax()
            particle = s.p[:,max_weight]
            
            d_world = s.rays2world(particle,d_body,head_angle=head_angle,neck_angle=neck_angle,angles=s.lidar_angles)
            cell_ind_x,cell_ind_y = s.map.grid_cell_from_xy(d_world[0],d_world[1])
            
            pos_indx,pos_indy = s.map.grid_cell_from_xy(particle[[0]],particle[[1]])       
            contours = np.vstack((cell_indy,cell_indx))
            contours = np.hstack((contours,np.array([pos_indy,pos_indx])))
            mask = np.zeros(s.map.log_odds.shape)
            
            cv.drawContours(image=mask,contours = [contours.T],contourIdx = -1,color = 1,thickness = -1)
            s.map.log_odds[mask==1] += s.lidar_log_odds_free

            s.map.log_odds[cell_indx,cell_indy]+=(-s.lidar_log_odds_free+s.lidar_log_odds_occ)
